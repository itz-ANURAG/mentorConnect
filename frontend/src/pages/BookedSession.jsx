...
const BookedSessions = () => {
  const [sessions, setSessions] = useState([]); // State to store booked sessions

  // State for filtering sessions based on date range
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");
  const [filterVisible, setFilterVisible] = useState(false); // Toggle visibility of the filter dropdown

  // Function to filter sessions by date range
  const filterSessions = () => {
    if (!startDate || !endDate) return sessions; // Return all sessions if no date is selected
  ...
  // Function to cancel a session by ID
  const cancelSession = (id) => {
    setSessions(sessions.filter((session) => session.id !== id)); // Remove the session with the given ID
  };
