...
const UpcomingSessions = ({ mentorId }) => {
    ...
    useEffect(() => {
        const fetchSessions = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/sessions/${id}/upcoming-sessions`, {
                    headers: {
    ...
    const cancelSession = async (sessionId) => {
        try {
            await axios.delete(`http://localhost:3000/sessions/${id}/cancel-session/${sessionId}`);
    ...
    const sortedSessions = [...filteredSessions].sort((a, b) => {
        if (sortOrder === 'asc') {
            ...
            return new Date(b.date) - new Date(a.date) || b.time.localeCompare(a.time);
        }
    });
                    ...
                    <tbody>
                        {currentSessions.length > 0 ? (
                            currentSessions.map((session) => (
                                <tr key={session.sessionId} className="hover:bg-gray-700 transition duration-300">
                            ...
                            <tr>
                                <td colSpan="5" className="p-4 text-center">No sessions found</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            <div className="flex justify-center mt-4">
                {Array.from({ length: totalPages }, (_, index) => (
                    <button
                        ...
                        className={`px-4 py-2 mx-1 rounded ${currentPage === index + 1 ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
                    >
                        {index + 1}
                    </button>
                ))}
